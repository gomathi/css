...
4. orders.json, contains 132 orders provided as part of the exercise.
5. css.log, a sample log generated by the fulfillment daemons during the test run with 132 orders.
6. orders_delivery_with_low_pickup_delay.csv, provides info on 132 orders how it was served(expired?, served?) during fulfillment service's run with low pickup delay(threads were picking up the orders from shelf with 2s to 10s delay.)
7.orders_delivery_with_high_pickup_delay.csv, provides info on 132 orders how it was served(expired?, served?) during fulfillment service's run with low pickup delay(threads were picking up the orders from shelf with 20s to 25s delay.)
